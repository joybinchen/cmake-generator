    ...
    def relpath(self, path):
        if self.directory:
            if path == self.directory:
                ...
                return path[len(self.directory):]
        return path
    ...
    def relpath(self, path):
        if self.directory:
            if path == self.directory:
                ...
                return path[len(self.directory):]
            elif path.startswith(self.db.directory[:-1]):
                pear_dir = self.db.relpath(path)
                my_dir = self.db.relpath(self.directory)
                return ('../' * len(my_dir.split('/'))) + pear_dir
        return path

    def get_include_path(self, include_path):
        result = os.path.relpath(include_path, self.directory)
        relpath_of_root = os.path.relpath(include_path, self.db.directory)
        # For paths that are not below the source root write the absolute path
        # to the CMakeLists instead of ../../../usr/local/include
        if os.path.isabs(include_path) and relpath_of_root.startswith(os.path.pardir):
            return include_path
        return result
